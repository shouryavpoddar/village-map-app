...
import AllVillagesSidebar from './AllVillagesSidebar';
import { parsePlotNumbersFromFile } from '../helpers/parsePlotNumbers';
import { Eyebrow, Select, Input } from '../../ui';

// `allVillages` is only set when the combined "All Villages" entry is
// selected in the town picker below - in that mode this renders
// AllVillagesSidebar instead of the normal per-plot editing panels (that
// view is read-only across every village's plots, so none of the
// rename/delete/color/group affordances apply).
export default function Sidebar({ engine, search, towns, townKey, onTownChange, maps, mapKey, onMapChange, loading, allVillages }) {
  const {
    plotCount, selectedPlot, renameLabel, deletePlot, setPlotColor, setPlotRealArea, saveStatus, plotsRef,
  ...
  return (
    <div className="flex-none w-[320px] bg-panel border-l border-line flex flex-col h-screen max-[760px]:w-full max-[760px]:h-[46vh] max-[760px]:order-2 max-[760px]:border-l-0 max-[760px]:border-t max-[760px]:border-line">
      ...
      {allVillages ? (
        <AllVillagesSidebar
          villages={allVillages.villages}
          loading={allVillages.loading}
          arrangement={allVillages.arrangement}
